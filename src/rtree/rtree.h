...
// template <typename T>
// class RTreeNode {
//     unsigned int num_keys;  // Number of keys currently in the node
//     MBR* mbrs;              // Array of mbrs
//     RTreeNode<T>* children;
//     bool leaf;  // True if node is a leaf
//     T* elem;

//    public:
//     RTreeNode() : num_keys(0), mbrs(nullptr), children(nullptr), is_leaf(false), elem(nullptr) {}
//     RTreeNode(std::vector<MBR>&, std::vector<RTreeNode<T>>&);
//     ~RTreeNode();
//     unsigned int size() const;
//     const std::pair<const MBR&, const RTreeNode<T>&> operator[](int) const;
//     std::pair<MBR&, RTreeNode<T>&> operator[](int);
//     bool is_leaf() const;
//     T* get_element(int);
//     const T* get_element(int) const;
// };

// template <typename T>
// class RTreeNodeIterator {
//     RTreeNode<T>* node;

//    public:
//     std::pair<const MBR&, T&> operator*();
//     const std::pair<const MBR&, const T&> operator*() const;
//     RTreeNodeIterator<T>& operator++();
//     const RTreeNodeIterator<T>& operator++() const;
//     RTreeNodeIterator<T>& operator++(int);
//     const RTreeNodeIterator<T>& operator++(int) const;
// };
...
//    public:
//     RTreeNodeIterator operator*();
//     const RTreeNodeIterator operator*() const;
//     RTreeIterator<T>& operator++();
//     const RTreeIterator<T>& operator++() const;
//     RTreeIterator<T>& operator++(int);
//     const RTreeIterator<T>& operator++(int) const;
// };

// template <typename T>
// class RTree {
//     const RTreeIterator<T> queryImpl(const std::function<bool(const T&)>& pred);

//     static std::vector<MBR> search(const RTreeNode<T> root, const std::vector<double>&
//     rectangle);
...
//     RTreeIterator<T> query(Predicate);

//     template <typename Predicate>
//     const RTreeIterator<T> query(const Predicate);

//     RTreeIterator<T> begin();
//     RTreeIterator<T> end();

//     const RTreeIterator<T> begin() const;
//     const RTreeIterator<T> end() const;
...
#define RTREE_TEMPLATE                                                                        \
    template <class DATATYPE, class ELEMTYPE, int NUMDIMS, class ELEMTYPEREAL, int TMAXNODES, \
              int TMINNODES>
...
///
template <class DataT, class ElemT, int NDims, class ElemTReal = ElemT, int TMAXNODES = 8,
          int TMINNODES = TMAXNODES / 2>
class RTree {
    static_assert(std::numeric_limits<ElemTReal>::is_iec559,
   ...
   public:
    ...
    RTree& insert(const ElemT a_min[NDims], const ElemT a_max[NDims], const DataT& a_dataId);

    /// Remove entry (traverse the whole tree and removes every occurrence)
    /// \param a_dataId Positive Id of data.  Maybe zero, but negative numbers not allowed.
    RTree& remove(const DataT& a_dataId);

    /// Remove entry (only if inside the given rect)
    /// \param a_min Min of bounding rect
    /// \param a_max Max of bounding rect
    /// \param a_dataId Positive Id of data.  Maybe zero, but negative numbers not allowed.
    RTree& remove(const ElemT a_min[NDims], const ElemT a_max[NDims], const DataT& a_dataId);

    /// Find all within search rectangle
    /// \param a_min Min of search bounding rect
    /// \param a_max Max of search bounding rect
    /// \param a_searchResult Search result array.  Caller should set grow size. Function will
    /// reset, not append to array.
    /// \param a_resultCallback Callback function to return result.  Callback should return 'true'
    /// to continue searching
    /// \param a_context User context to pass as parameter to a_resultCallback
    /// \return Returns the number of entries found
    int search(const ElemT a_min[NDims], const ElemT a_max[NDims],
               std::function<bool(const DataT&)> callback) const;

    /// Find the nearest neighbors
    /// \param a_min Min of search bounding rect
    /// \param a_max Max of search bounding rect
    /// \param a_resultCallback Callback function to return result.  The Callback takes both the
    /// resulting data point and the calculated square distance. Callback should return 'true' to
    /// continue searching
    /// \return Returns the number of entries found
    size_t nn_search(const ElemT a_min[NDims], const ElemT a_max[NDims],
                     std::function<bool(const DataT&, ElemT)> callback) const;
    ...
    class Iterator {
       ...
       public:
        ...
        void get_bounds(DataT a_min[NDims], DataT a_max[NDims]) {
            assert(!is_null());
            StackElement& curTos = m_stack[m_tos - 1];
    ...
    void get_first(Iterator& a_it) {
        a_it.init();
        Node* first = m_root;
        while (first) {
            if (first->is_internal_node() && first->m_count > 1) {
    ...
    void get_next(Iterator& a_it) { ++a_it; }

    /// Is iterator NULL, or at end?
    bool is_null(Iterator& a_it) { return a_it.IsNull(); }

    /// Get object at iterator position
    DataT& get_at(Iterator& a_it) { return *a_it; }

   protected:
    /// Minimal bounding rectangle (n-dimensional)
    struct Rect {
        ElemT m_min[NDims];  ///< Min dimensions of bounding box
        ElemT m_max[NDims];  ///< Max dimensions of bounding box
    };
    ...
    struct Branch {
        ...
        DataT m_data;          ///< Data Id
    };

    /// Node for each branch level
    struct Node {
        bool is_internal_node() { return (m_level > 0); }  // Not a leaf, but a internal node
        bool is_leaf() { return (m_level == 0); }          // A leaf, contains data
    ...
    struct PartitionVars {
        ...
        int m_branchCount;
        Rect m_coverSplit;
        ElemTReal m_coverSplitArea;
    };

    Node* alloc_node();
    void free_node(Node* a_node);
    void init_node(Node* a_node);
    void init_rect(Rect* a_rect);
    bool insert_rect_rec(const Branch& a_branch, Node* a_node, Node** a_newNode, int a_level);
    bool Insert_rect(const Branch& a_branch, Node** a_root, int a_level);
    Rect node_cover(Node* a_node);
    bool add_branch(const Branch* a_branch, Node* a_node, Node** a_newNode);
    void disconnect_branch(Node* a_node, int a_index);
    int pick_branch(const Rect* a_rect, Node* a_node);
    Rect combine_rect(const Rect* a_rectA, const Rect* a_rectB);
    void split_node(Node* a_node, const Branch* a_branch, Node** a_newNode);
    ElemTReal rect_spherical_volume(Rect* a_rect);
    ElemTReal rect_volume(Rect* a_rect);
    ElemTReal calc_rect_volume(Rect* a_rect);
    void get_branches(Node* a_node, const Branch* a_branch, PartitionVars* a_parVars);
    void choose_partition(PartitionVars* a_parVars, int a_minFill);
    void load_nodes(Node* a_nodeA, Node* a_nodeB, PartitionVars* a_parVars);
    void init_par_vars(PartitionVars* a_parVars, int a_maxRects, int a_minFill);
    void pick_seeds(PartitionVars* a_parVars);
    void classify(int a_index, int a_group, PartitionVars* a_parVars);
    bool remove_rect(Rect* a_rect, const DataT& a_id, Node** a_root);
    bool remove_rect_rec(Rect* a_rect, const DataT& a_id, Node* a_node, ListNode** a_listNode);
    ListNode* alloc_list_node();
    void free_list_node(ListNode* a_listNode);
    bool overlap(Rect* a_rectA, Rect* a_rectB) const;
    ElemT square_distance(Rect const& a_rectA, Rect const& a_rectB) const;
    void reinsert(Node* a_node, ListNode** a_listNode);
    bool search(Node* a_node, Rect* a_rect, int& a_foundCount,
                std::function<bool(const DataT&)> callback) const;
    void remove_all_rec(Node* a_node);
    void reset();
    void count_rec(Node* a_node, int& a_count);

    bool save_rec(Node* a_node, RTFileStream& a_stream);
    bool load_rec(Node* a_node, RTFileStream& a_stream);
    void copy_rec(Node* current, Node* other);

    Node* m_root;                  ///< Root of tree
    ElemTReal m_unitSphereVolume;  ///< Unit sphere constant for required number of dimensions

   public:
    // return all the AABBs that form the RTree
    std::vector<Rect> list_tree() const;
};
...
RTREE_TEMPLATE
void RTREE_QUAL::Insert(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
...
RTREE_TEMPLATE
void RTREE_QUAL::Remove(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS],
                        const DATATYPE& a_dataId) {
#ifdef _DEBUG
    for (int index = 0; index < NUMDIMS; ++index) {
        RTREE_ASSERT(a_min[index] <= a_max[index]);
...
RTREE_TEMPLATE
bool RTREE_QUAL::InsertRectRec(const Branch& a_branch, Node* a_node, Node** a_newNode,
                               int a_level) {
...
ELEMTYPEREAL RTREE_QUAL::RectSphericalVolume(Rect* a_rect) {
    ...
    for (int index = 0; index < NUMDIMS; ++index) {
        ELEMTYPEREAL halfExtent =
            ((ELEMTYPEREAL)a_rect->m_max[index] - (ELEMTYPEREAL)a_rect->m_min[index]) *
            (ELEMTYPEREAL)0.5;
...
class RTreeGutman {
   ...
   public:
    struct Rectangle {
        ...
        static bool overlap(const Rectangle&, const Rectangle&);
        static Rectangle calc_mbr(const Rectangle&, const Rectangle&);
        static double calc_mbr_area(const Rectangle&, const Rectangle&);
        double enlargement_needed(const Rectangle&) const;
        double area() const;
        static bool equal(const Rectangle&, const Rectangle&);
    };
    ...
    void insert(Rectangle& r, T* elem = nullptr) {
        if (root == nullptr) {
            root = new Block{
                .is_leaf = true, .parent = nullptr, .count = 1, .nodes = std::vector<Node*>()};
            root->nodes.push_back(
                new Node{.elem = elem, .mbr = r, .parent = root, .children = nullptr});
            return;
        }

        auto leaf = choose_leaf();
        leaf->nodes.push_back(
            new Node{.elem = elem, .mbr = r, .children = nullptr, .parent = leaf});
        leaf->count++;
        if (leaf->count > M) {
            split(leaf);
        }
    ...
    void split(Block* t) {
        ...
        for (int i = 0; i < t->count; i++) {
            ...
            if (left_parent->mbr.enlargement_needed(ptr->mbr) <
                right_parent->mbr.enlargement_needed(ptr->mbr)) {
